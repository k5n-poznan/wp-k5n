...
class WP_K5N_Subscribers_List_Table extends WP_List_Table {
    ...
    function __construct() {
        ...
        $this->data = $wpdb->get_results("SELECT * FROM `{$table_prefix}k5n_subscribes`", ARRAY_A);
    }
    ...
    function column_name($item) {
        ...
        //Build row actions
        $actions = array(
            'edit' => sprintf('<a href="?page=%s&action=%s&ID=%s">' . __('Edytuj', 'wp-k5n') . '</a>', $_REQUEST['page'], 'edit', $item['ID']),
            'delete' => sprintf('<a href="?page=%s&action=%s&ID=%s">' . __('Usuń', 'wp-k5n') . '</a>', $_REQUEST['page'], 'delete', $item['ID']),
        );

        //Return the title contents
        return sprintf('%1$s %4$s',
                /* $1%s */ $item['name'],
                /* $2%s */ $item['surname'],
                /* $3%s */ $item['ID'],
    ...
    function get_columns() {
        $columns = array(
            ...
            'name' => __('Imie', 'wp-k5n'),
            'surname' => __('Nazwisko', 'wp-k5n'),
            'mobile' => __('Nr telefonu', 'wp-k5n'),
            'group_ID' => __('Grupa', 'wp-k5n'),
            'date' => __('Data', 'wp-k5n'),
            'status' => __('Status', 'wp-k5n'),
            'activate_key' => __('Kod aktywacyjny', 'wp-k5n'),
        );
    ...
    function get_sortable_columns() {
        $sortable_columns = array(
            ...
            'surname' => array('surname', false), //true means it's already sorted
    ...
    function process_bulk_action() {
        ...
        // Search action
        if (isset($_GET['s'])) {
            $this->data = $wpdb->get_results($wpdb->prepare("SELECT * from `{$table_prefix}k5n_subscribes` WHERE name LIKE %s OR mobile LIKE %s;", '%' . $wpdb->esc_like($_GET['s']) . '%', '%' . $wpdb->esc_like($_GET['s']) . '%'), ARRAY_A);
        }

        // Bulk delete action
        if ('bulk_delete' == $this->current_action()) {
            foreach ($_GET['id'] as $id) {
                $wpdb->delete($table_prefix . "k5n_subscribes", array('ID' => $id));
            }

            $this->data = $wpdb->get_results("SELECT * FROM `{$table_prefix}k5n_subscribes`", ARRAY_A);
            echo '<div class="updated notice is-dismissible below-h2"><p>' . __('Pozycja usunieta.', 'wp-k5n') . '</p></div>';
        }

        // Single delete action
        if ('delete' == $this->current_action()) {
            $wpdb->delete($table_prefix . "k5n_subscribes", array('ID' => $_GET['ID']));
            $this->data = $wpdb->get_results("SELECT * FROM `{$table_prefix}k5n_subscribes`", ARRAY_A);
            echo '<div class="updated notice is-dismissible below-h2"><p>' . __('Pozycja usunieta.', 'wp-k5n') . '</p></div>';
        }
    }
    ...
    function prepare_items() {
         ...
         */
        $per_page = isset($this->option['subscribers_page_size']) ? $this->option['subscribers_page_size'] : '15';
