...
if (isset($_POST['wps_import'])) {
    if (!$_FILES['wps-import-file']['error']) {
        ...
        if ($result) {
            echo "<div class='updated'><p>" . sprintf(__('<strong>%s</strong> items was successfully added.', 'wp-k5n'), count($total_submit)) . "</div></p>";
        }

        if ($duplicate) {
            echo "<div class='error'><p>" . sprintf(__('<strong>%s</strong> Mobile numbers Was repeated.', 'wp-k5n'), count($duplicate)) . "</div></p>";
        }
    } else {
